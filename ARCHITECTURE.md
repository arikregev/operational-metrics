...
1. **Versions changes-feed scheduler** ([VersionsChangesFeedService.pollAllRegistries](/Users/arikregev/Documents/operational-metrics/src/main/java/com/example/operationalmetrics/service/VersionsChangesFeedService.java)). Cron-fired every 15 min (default). For each registry the operator listed in `metrics.versions-feed.registries`, paginates `GET /api/v1/registries/{r}/packages?sort=latest_release_published_at&order=desc` until results dip below the per-registry watermark stored in `versions_feed_cursor`. Names from the response are bulk-intersected against the local `package` table via `purl_canonical = ANY(...)`, and matched packages are queued for `analyze()` through a Semaphore-bounded virtual-thread executor with its own token-bucket rate cap. **Primary path** for sub-15-min freshness — at ~0.1–1% daily registry churn this lands ~9K–90K analyze calls/day on a comfortable single-source budget.
2. **Versions sweep scheduler** ([VersionsSyncService.runSweep](/Users/arikregev/Documents/operational-metrics/src/main/java/com/example/operationalmetrics/service/VersionsSyncService.java)). Cron-fired (default daily 3 AM). Walks `package` rows due to be polled (`latest_versions_polled_at` older than `metrics.versions.staleness-days`), runs `analyze()` for each within its own Semaphore + token-bucket pair (separate from the feed's so they don't starve each other). **Safety net** — catches drift if a feed window was missed (registry outage, our outage, watermark advance bug). At ~9M packages with `rate-budget-per-second=4`, the sweep alone gives ~`staleness-days`-week coverage cycles; with the feed running too, the sweep mostly does no work.
3. **Inline backfill of latest version.** After collectors finish for a package, `MetricsOrchestrator` calls `findOrFetchByVersion(...)` for `merged.lastReleaseVersion` so Snyk's authoritative `published_at` lands in `package_version` for the most-relevant version. 1 cheap call per metric refresh.
4. **API caller asks about a specific version.** `MetricsResource.getByPurl` calls `MetricsQueryService.findByPurl` which extracts the version segment from the PURL and triggers Flow B. The result lands in `MetricsResponse.versionInfo` with `daysSinceRelease` and `daysOlderThanLatest` computed.

### Cursor & watermark semantics

`versions_feed_cursor` keeps `(registry, last_polled_at, last_seen_release_at)`. The `last_seen_release_at` field is the watermark — any release strictly older than it is below the threshold and stops pagination. After each poll: `last_polled_at` is bumped unconditionally (so we record we tried), and `last_seen_release_at` is `COALESCE`'d in the upsert — when no fresh refs were seen, the existing watermark is preserved rather than being wiped. On a registry's first poll (no row), the watermark is seeded as `now() - metrics.versions-feed.initial-lookback-hours`. A blown-up page fetch logs and breaks pagination but still persists the cursor (failure must not cause re-walking the same window infinitely).
